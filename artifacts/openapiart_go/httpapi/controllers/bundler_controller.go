...
func (ctrl *bundlerController) SetConfig(w http.ResponseWriter, r *http.Request) {
	var item openapiart.PrefixConfig
	if r.Body != nil {
		body, readError := io.ReadAll(r.Body)
		if body != nil {
			item = openapiart.NewPrefixConfig()
...
func (ctrl *bundlerController) responseSetConfigError(w http.ResponseWriter, errorKind openapiart.ErrorKindEnum, rsp_err error) {
	var result openapiart.Error
	...
	if rErr, ok := rsp_err.(openapiart.Error); ok {
		result = rErr
	} else {
		result = openapiart.NewError()
...
func (ctrl *bundlerController) UpdateConfiguration(w http.ResponseWriter, r *http.Request) {
	var item openapiart.UpdateConfig
	if r.Body != nil {
		body, readError := io.ReadAll(r.Body)
		if body != nil {
			item = openapiart.NewUpdateConfig()
			err := item.FromJson(string(body))
			if err != nil {
...
func (ctrl *bundlerController) responseUpdateConfigurationError(w http.ResponseWriter, errorKind openapiart.ErrorKindEnum, rsp_err error) {
	var result openapiart.Error
	var statusCode int32
	if errorKind == "validation" {
		statusCode = 400
...
func (ctrl *bundlerController) responseGetConfigError(w http.ResponseWriter, errorKind openapiart.ErrorKindEnum, rsp_err error) {
	var result openapiart.Error
	var statusCode int32
	if errorKind == "validation" {
		statusCode = 400
