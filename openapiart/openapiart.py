...
class OpenApiArt(object):
    ...
    def __init__(
        ...
        # TODO: doc dir should be for redocly ?
        ...
        self._protobuf_package_name = (
            protobuf_name if protobuf_name is not None else "protobuf"
        )
        svc_name = self._protobuf_package_name.capitalize() + "Service"
        self._proto_service = (
            proto_service if proto_service is not None else svc_name
        )
    ...
    def GeneratePythonSdk(
        self, package_name, extension_prefix=None, parent_dir=None
    ):
        ...
        - parent_dir (str): path of parent dir of SDK relative to artifact_dir

        Example
        -------
        ```
        art = Openapiart(api_files=["<list of open_api_file_path>"], artifact_dir="./", protobuf_name="openapi")
        art.GeneratePythonSdk(package_name="pyopenapi")
        ```
        ...
        self._extension_prefix = (
            extension_prefix if extension_prefix is not None else package_name
        )
        self._parent_dir = (
            parent_dir
            if parent_dir is not None
            else os.path.join(self._output_dir, "python")
        )
        ...
        if self._python_module_name is not None:
            ...
            python_ux = Generator(
                ...
                self._protobuf_package_name,
                output_dir=self._parent_dir,
                extension_prefix=self._extension_prefix,
            )
            python_ux.generate()
        try:
            python_sdk_dir = os.path.normpath(
                os.path.join(self._parent_dir, self._python_module_name)
            )
            process_args = [
                sys.executable,
                "-m",
    ...
    def GenerateGoSdk(self, package_dir, package_name, parent_dir=None):
        ...
        self._parent_dir = (
            parent_dir
            if parent_dir is not None
            else os.path.join(self._output_dir, "go")
        )
        self._generate_proto_file()
        ...
        if self._protobuf_package_name and self._go_sdk_package_dir:
            from .openapiartgo import OpenApiArtGo
    ...
    def _generate_proto_file(self):
        from .openapiartprotobuf import OpenApiArtProtobuf

        protobuf = OpenApiArtProtobuf(
            info=self._info,
            license=self._license,
            protobuf_package_name=self._protobuf_package_name,
            go_sdk_package_dir=self._go_sdk_package_dir,
