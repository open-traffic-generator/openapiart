...
def test_error_for_enums(api):
    ...
    expected_exception = "property response shall be one of these ['status_200', 'status_400', 'status_404', 'status_500'] enum, but got status_123 at PrefixConfig"
    ...
    expected_exception = "property d_values shall be one of these ['a', 'b', 'c'] enum, but got ['a', 'error'] at PrefixConfig"
    with pytest.raises(Exception) as execinfo:
        config.serialize()
    assert execinfo.value.args[0] == expected_exception
