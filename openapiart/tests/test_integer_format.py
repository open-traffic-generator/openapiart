...
def test_auto_assign_for_integers(default_config):
    value1 = default_config._TYPES.get("auto_int64_param")
    value2 = default_config._TYPES.get("auto_int64_list_param")
    value3 = default_config._TYPES.get("auto_uint64_param")
    value4 = default_config._TYPES.get("auto_uint64_list_param")
    value5 = default_config._TYPES.get("auto_uint32_param")
    value6 = default_config._TYPES.get("auto_uint32_list_param")

    assert value1.get("format") is not None
    assert value3.get("format") is not None
    assert value5.get("format") is not None
    assert value2.get("itemformat") is not None
    assert value4.get("itemformat") is not None
    assert value6.get("itemformat") is not None
    assert value1.get("format") == "int64"
    assert value2.get("itemformat") == "int64"
    assert value3.get("format") == "uint64"
    assert value4.get("itemformat") == "uint64"
    assert value5.get("format") == "uint32"
    assert value6.get("itemformat") == "uint32"

    default_config.auto_uint64_param = 65
    default_config.auto_uint64_list_param = [65]
    default_config.auto_uint32_param = 5675
    default_config.auto_uint32_list_param = [5675]
    default_config.auto_int64_param = -429496
    default_config.auto_int64_list_param = [-1]
    ...
    # negative test case
    error_msg = "got 9223372036854775808 of type <class 'int'> , expected min -9223372036854775808, expected max 9223372036854775807"
    default_config.auto_int64_param = 9223372036854775808
    with pytest.raises(Exception) as execinfo:
        default_config.serialize("dict")
    error_value = execinfo.value.args[0]
    assert error_msg in error_value
