...
func TestConstraintAndUnique(t *testing.T) {
	...
	prefix.ZObject().SetName("abc")
	prefix.XList().Add().SetName("abc")
	_, err := prefix.ToJson()
	assert.NotNil(t, err)
	assert.Contains(t, err.Error(), "abc already exists")
	prefix.XList().Items()[0].SetName("bcd")
	_, err = prefix.ToJson()
	assert.Nil(t, err)
